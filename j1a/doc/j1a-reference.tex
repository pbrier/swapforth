...
\longworddef{sramw}{( d a -- )}{Write data 'd' to SRAM address 'a'}
\longworddef{sramr}{( a -- d )}{Read data from SRAM address 'a' }
...
\item \GPIO{} GPIO connector \index{GPIO1}
\item UART \index{UART}
\item SRAM (64k x 16bit accessable)\index{SRAM}
\end{itemize}
...
\begin{framed}
\begin{Verbatim}
\ test the digital outputs, generate square wave on each pin
\ D0=max freq, D1=Fmax/2 .. D15=Fmax/2^15
: testio
$ffff 2 io!
begin
  65535 0 do
